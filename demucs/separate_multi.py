...
class FolderSet(Dataset):
    def __init__(self, path, out, audio_channels, samplerate):
        tracks = list(pathlib.Path(path).rglob('*.wav'))
        progressfile = os.path.join(out, 'progress.txt')
        if os.path.exists(progressfile):
            with open(progressfile, 'r', encoding='utf-8') as f:
                already_processed = [l.strip() for l in f.readlines()]
        else:
            already_processed = []
        self.progressfile = progressfile
        self.tracks = list(filter(lambda t: t not in already_processed, tracks))
        self.samplerate = samplerate
        self.audio_channels = audio_channels

    def __getitem__(self, item):
        track = self.tracks[item]
        wav = load_track(track, self.audio_channels, self.samplerate)

        ref = wav.mean(0)
        wav = (wav - ref.mean()) / ref.std()
        return {'audio': wav, 'track': str(track)}
...
def main():
    ...
    parser.add_argument("path", type=str, default='', help='Path to tracks')
    ...
    parser.add_argument("--num_workers",
                        default=1,
                        type=int,
                        help="Number of concurrent processes that will be spawned to process files.")
    ...
    dataset = FolderSet(args.path, out, model.audio_channels, model.samplerate)
    dataloader = DataLoader(dataset, batch_size=8, num_workers=2)
    print(f"{len(dataset)} total tracks. Separated tracks will be stored in {out.resolve()}")

    for batch in tqdm(dataloader):
        sources_batched = apply_model(model, batch['audio'], device=args.device, shifts=args.shifts,
        ...
        for j_, sources in enumerate(sources_batched):
            track = batch['track'][j_]
            track_folder = out / os.path.sep.join(track.split(os.path.sep)[-3:])[:-4]
            ...
            if args.stem is None:
                for source, name in zip(sources, model.sources):
                    ...
                    source = resample(source, model.samplerate, args.samplerate)
                    save_audio(source[0].unsqueeze(0), stem, **kwargs)
            else:
                ...
                other_stem = resample(other_stem, model.samplerate, args.samplerate)
                save_audio(other_stem[0].unsqueeze(0), stem, **kwargs)

            shutil.copy(track, track_folder)
            with open(dataset.progressfile, 'a', encoding='utf-8') as f:
                f.write(str(track) + "\n")
